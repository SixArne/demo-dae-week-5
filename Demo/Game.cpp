...
void Game::DrawPoly() const
{
	...
	matCenter = Matrix2x3::CreateTranslationMatrix(-m_LogoText.GetWidth() / 2, -m_LogoText.GetHeight() / 2);

	Matrix2x3 matWorld{matTranslate * matRotate * matScale * matCenter};
	std::vector<Point2f> transformedVerts;
	transformedVerts = matWorld.Transform(vertices);

	utils::DrawPolygon(transformedVerts, true, 5.0f);
}
